...
Supports both local Streamlit (using .env credentials) and Snowflake Streamlit native app environments.
"""
...
def _is_snowflake_environment():
    """
    Detect if running inside Snowflake Streamlit native app.
    Returns True if SNOWFLAKE_ENV=STREAMLIT, False otherwise.
    """
    return os.getenv("SNOWFLAKE_ENV", "").upper() == "STREAMLIT"
...
def get_snowflake_connection():
    ...
    - If SNOWFLAKE_ENV=STREAMLIT: use Streamlit's built-in connection
    - Otherwise: use snowflake.connector with .env credentials
    """
    if _is_snowflake_environment():
        # Running in Snowflake Streamlit - use built-in connection
...
def execute_query(query: str) -> pd.DataFrame:
    ...
    Works in both local and Snowflake Streamlit environments.
    """
    if _is_snowflake_environment():
        # Snowflake Streamlit connection has query method
        conn = get_snowflake_connection()
        return conn.query(query)
    else:
        # Local connection uses cursor
        conn = get_snowflake_connection()
        try:
            cur = conn.cursor()
            cur.execute(query)
...
def create_tables_if_not_exist():
    ...
    if _is_snowflake_environment():
        ...
        try:
            ...
            conn.query("CREATE DATABASE IF NOT EXISTS INTELLISTOCK_DB")
            conn.query("USE DATABASE INTELLISTOCK_DB")
            conn.query("CREATE SCHEMA IF NOT EXISTS PUBLIC")
...
def load_csv_data(csv_path):
    ...
    try:
        ...
        if _is_snowflake_environment():
            ...
            # Clear existing data
            conn.query("TRUNCATE TABLE IF EXISTS INVENTORY")
            
            # Insert data using session.write_pandas
            # Get the underlying session from the connection
            import streamlit as st
            ...
            session.write_pandas(
                ...
                database=os.getenv("SNOWFLAKE_DATABASE", "INTELLISTOCK_DB"),
